...
public class Scheduler {
	...
	private void handleShow() {
		...
		for (int i=0; i<n; i++) {
			if(events[i].isRelevant(theDate))	// event 타입에는 isRelevant가 없으므로 문제가 생김
				System.out.println("    " + events[i].toString());	// toString은  Oject클래스에 toString이 있으므로 
	...
	private void addEvent(Event ev) {
		if (n >= capacity)
			reallocate();
		events[n++] = ev;	// 여기서 다형성이 적용됨. events는 event타입이지만 ev는 evnet타입의 서브클래스인 onedatevent 타입임
